...
from utils.constants import DEFAULT_FPS, DEFAULT_RESOLUTION, MAX_CAMERAS
...
class VideoStream:
    ...
    def __init__(self, camera_id: str, video_path: str, stream_name: str):
        ...
        self.logger = setup_logger(f"video_stream_{camera_id}")
        self.logger.info(f"📹 Video stream initialized: {stream_name} ({camera_id})")
    ...
    def start_stream(self):
        ...
        try:
            ...
            self.logger.info(f"✅ Stream started: {self.stream_name} ({self.fps} FPS, {self.frame_count} frames)")
            return True
    ...
    def stop_stream(self):
        ...
        if self.cap:
            self.cap.release()
        self.logger.info(f"🛑 Stream stopped: {self.stream_name}")
...
class VideoStreamingSystem:
    ...
    def __init__(self):
        self.logger = setup_logger("video_streaming_system")
        self.streams = {}
        self.stream_threads = {}
        ...
        self.logger.info("✅ Video streaming system initialized")
    
    def setup_camera_streams(self, dataset_path: str = None) -> bool:
        ...
        try:
            ...
            if not dataset_dir.exists():
                self.logger.error(f"❌ Dataset directory not found: {dataset_path}")
                return False
            ...
            for camera_id, video_path in camera_videos.items():
                if video_path:
                    ...
                    if stream.start_stream():
                        ...
                        self.logger.info(f"✅ Camera stream setup: {camera_id} -> {stream_name}")
                    else:
                        self.logger.error(f"❌ Failed to setup stream: {camera_id}")
            
            self.logger.info(f"📹 Setup {len(self.streams)} camera streams")
            return len(self.streams) > 0
            
        except Exception as e:
            self.logger.error(f"❌ Error setting up camera streams: {e}")
            return False
    ...
    def start_all_streams(self):
        ...
        self.logger.info(f"🚀 Started {len(self.streams)} video streams")
    ...
    def stop_all_streams(self):
        ...
        self.logger.info("🛑 All video streams stopped")
...
def setup_video_streams(dataset_path: str = None) -> bool:
