...
from utils.constants import EVIDENCE_RETENTION_DAYS, MAX_EVIDENCE_SIZE, EVIDENCE_FORMATS
...
class EvidenceCollector:
    ...
    def __init__(self, evidence_dir: str = None):
        self.logger = setup_logger("evidence_collector")
        ...
        self.clip_duration_before = settings.CLIP_DURATION_BEFORE
        self.clip_duration_after = settings.CLIP_DURATION_AFTER
        self.max_clip_duration = 60  # maximum clip length
        
        self.logger.info(f"✅ Evidence collection system initialized")
        self.logger.info(f"📁 Evidence directory: {self.evidence_dir.absolute()}")
    ...
    def load_evidence_database(self) -> Dict:
        ...
        if self.evidence_db_file.exists():
            try:
                ...
                self.logger.error(f"❌ Error loading evidence database: {e}")
    ...
    def save_evidence_database(self):
        ...
        try:
            ...
            self.logger.info("✅ Evidence database saved")
        except Exception as e:
            self.logger.error(f"❌ Error saving evidence database: {e}")
    ...
    def extract_video_clip(self, source_video_path: str, detection: Dict) -> Optional[str]:
        ...
        try:
            ...
            if duration == 0:
                self.logger.error(f"❌ Invalid video duration: {source_video_path}")
                return None
            ...
            if result.returncode == 0 and output_path.exists():
                self.logger.info(f"✅ Video clip extracted: {output_path.name}")
            ...
            self.logger.error(f"❌ Error extracting video clip: {e}")
            return None
    ...
    def save_evidence_metadata(self, metadata: Dict):
        ...
        try:
            ...
            self.logger.info(f"✅ Evidence metadata saved: {metadata_file.name}")
            
        except Exception as e:
            self.logger.error(f"❌ Error saving evidence metadata: {e}")
    
    def collect_evidence(self, detection: Dict, source_video_path: Optional[str] = None) -> Dict:
        """Main evidence collection function"""
        self.logger.info(f"🔍 Collecting evidence for {detection.get('threat_type')} detection...")
        ...
        if source_video_path and os.path.exists(source_video_path):
            ...
            self.logger.warning(f"⚠️ No source video provided or file doesn't exist: {source_video_path}")
    ...
    def cleanup_old_evidence(self, days_to_keep: int = EVIDENCE_RETENTION_DAYS):
        ...
        self.logger.info(f"🧹 Cleaned up {removed_count} old evidence files")
